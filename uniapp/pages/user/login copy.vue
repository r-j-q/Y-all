<template>
	...
	<view class="backgroundbg" >
		...
		<view class="ic_splash_logowebp_height" :style="{background: 'url('+loginBg+')'}" >
			<view class="ic_splash_logowebp">
		 	<image :src="ic_splash_logowebp" mode=""></image>
			</view>
		</view>
	...
	<view class="login-wrap cu-dialog form-wrap  safe-area-inset-bottom">
			...
			<view class="form-item u-border-bottom">
				<view class="item-content u-flex u-col-center">
					...
					<button class="u-reset-button code-btn code-btn-start"
						...
						:class="{ 'code-btn-end': form['smsLogin'].data.isMobileEnd }" @tap="getSmsCode('mobilelogin')">
						{{ codeText }}
					</button>
				</view>
			...
			<view class="form-item u-border-bottom">
				<view class="item-content u-flex u-col-center">
					...
					<u-input class="u-m-r-10 u-flex-1" @blur="checkValue($event, 'code')"
						@input="checkValue($event, 'code')" placeholder="请输入验证码" :placeholderStyle="placeholderStyle"
						v-model="form['smsLogin'].data.code" type="number">
					</u-input>
		...
		<view v-if="authType === 'register'">
			...
			<view class="form-item u-border-bottom">
				<view class="item-content u-flex u-col-center">
					...
					<button class="u-reset-button code-btn code-btn-start"
						...
						:class="{ 'code-btn-end': form['register'].data.isMobileEnd }" @tap="getSmsCode('register')">
						{{ codeText }}
					</button>
				</view>
			...
			<view class="form-item u-border-bottom">
				<view class="item-content u-flex u-col-center">
					...
					<u-input class="u-m-r-10 u-flex-1" @blur="checkValue($event, 'password')"
						@input="checkValue($event, 'password')" placeholder="请输入密码" :placeholderStyle="placeholderStyle"
			...
			<view class="form-item u-border-bottom">
				<view class="item-content u-flex u-col-center">
					...
					<u-input class="u-m-r-10 u-flex-1" @blur="checkValue($event, 'code')"
						@input="checkValue($event, 'code')" placeholder="请输入验证码" :placeholderStyle="placeholderStyle"
						v-model="form['register'].data.code" type="number">
					</u-input>
			...
			<view class="form-item u-border-bottom">
				<view class="item-content u-flex u-col-center">
					...
					<u-input class="u-m-r-10 u-flex-1" @blur="checkValue($event, 'Ycode')"
						@input="checkValue($event, 'Ycode')" placeholder="请输入邀请码 (必填)" :placeholderStyle="placeholderStyle"
						v-model="form['register'].data.Ycode" type="number">
					</u-input>
		...
		<view v-if="authType === 'forgotPwd'">
			...
			<view class="form-item u-border-bottom">
				<view class="item-content u-flex u-col-center">
					...
					<button class="u-reset-button code-btn code-btn-start"
						...
						:class="{ 'code-btn-end': form['forgotPwd'].data.isMobileEnd }" @tap="getSmsCode('resetpwd')">
						{{ codeText }}
					</button>
				</view>
			...
			<view class="form-item u-border-bottom">
				<view class="item-content u-flex u-col-center">
					...
					<u-input class="u-m-r-10 u-flex-1" @blur="checkValue($event, 'code')"
						@input="checkValue($event, 'code')" placeholder="请输入验证码" :placeholderStyle="placeholderStyle"
						v-model="form['forgotPwd'].data.code" type="number">
					</u-input>
				</view>
		...
		<view v-if="authType === 'bindMobile'">
			...
			<view class="form-item u-border-bottom">
				<view class="item-content u-flex u-col-center">
					<view class="item-title">手机号</view>
					<u-input class="u-m-r-10 u-flex-1" @blur="checkValue($event, 'mobile')"
						@input="mobileInput($event, 'mobile')" placeholder="请输入手机号" :placeholderStyle="placeholderStyle"
						v-model="form['bindMobile'].data.mobile" type="number">
					</u-input>
					<button class="u-reset-button code-btn code-btn-start"
			...
			<view class="form-item u-border-bottom">
				<view class="item-content u-flex u-col-center">
					...
					<u-input class="u-m-r-10 u-flex-1" @blur="checkValue($event, 'code')"
						@input="checkValue($event, 'code')" placeholder="请输入验证码" :placeholderStyle="placeholderStyle"
						v-model="form['bindMobile'].data.code" type="number">
					</u-input>
		...
		<view v-if="authType === 'accountLogin' || authType === 'smsLogin'"
			...
			<!-- <image v-if="['App', 'wxOfficialAccount', 'wxMiniProgram'].includes(platform)" class="auto-login-img"
				@tap="thirdLogin('wechat')" :src="$IMG_URL + '/imgs/auto_login_wx.png'"></image> -->
			<!-- 支付宝 -->
			<!-- <image
			...
			<u-checkbox v-model="protocol" shape="circle" active-color="#E9B461">
				<view class="agreement-text tcp-text u-flex u-col-center">
					...
					<view class="tcp-text u-flex u-col-center">
						<view @tap.stop="navigateToXieyi(initShop.user_protocol || 0)">《用户协议》</view>
						与
						<view @tap.stop="navigateToXieyi(initShop.privacy_protocol || 0)">《隐私协议》</view>
					</view>
		 ...
		 mounted() {
			...
			if(this.showAuth) {
				setTimeout(()=>{
					...
					console.log("====6666666666666===>",this.showAuth)
			}
		 },
		methods: {
				...
				that
					.$http(
						'user.register', {
							...
							parent_user_id: that.form['register'].data.Ycode,
  ...
  .ic_splash_logowebp_height{
	...
	.login-wrap {
		...
		.login-btn-start {
			width: 158rpx;
			line-height: 56rpx;
			background: linear-gradient(90deg, #7C75F5, #7C75F5);
			border-radius: 28rpx;
			font-size: 26rpx;
			font-weight: 500;
			color: #ffffff;
